...
Finnhub credentials have since been configured locally and the explicitly approved public-portfolio quote refresh succeeded. Yahoo supplies free daily history independently. Optional AI remains unconfigured. Complete historical cash flows are absent, so lifetime contribution returns remain unavailable; forward TWR can begin with a new verified interval. No historical positions or missing flows are invented.

Forward-tracking follow-up: all 80 backend tests pass, including mixed-date quote coverage, cash-only unknown P/L, private/idempotent Replay capture, verified-period benchmark alignment and retention across pending later intervals. Both Chrome desktop and 390px mobile journeys pass on a separate fictional database. The live Overview was visually inspected with dated quote coverage; Replay shows stored current positions and the manual capture control. The local API runs approved quote refreshes followed by snapshots; Google sync is conditional on runtime credentials. Production database parity is covered by the repository's PostgreSQL CI job.
